    ...
    <div class="min-h-screen bg-zinc-950">
        ...
        <div class="relative overflow-hidden">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
                ...
                <nav class="flex items-center gap-2 text-sm text-zinc-500 mb-6">
                    <a href="{{ route('home') }}" class="hover:text-(--color-accent-400) transition-colors">
                        {{ __('common.home') }}
                    </a>
                    <i class="fa-solid fa-chevron-right text-xs"></i>
                    <a href="{{ route('clips.list') }}" class="hover:text-(--color-accent-400) transition-colors">
                        {{ __('clips.browse') }}
                    </a>
                    <i class="fa-solid fa-chevron-right text-xs"></i>
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-3">
                                ...
                                <h1 class="text-zinc-100 text-3xl lg:text-4xl font-bold leading-tight">
                                    {{ $clip->title }}
                                </h1>
                            </div>
                            ...
                            <div class="relative bg-zinc-900 border border-zinc-800/50 rounded-2xl overflow-hidden shadow-2xl">
                                ...
                                <div class="absolute top-4 right-4">
                                    <x-ui.button 
                                        ...
                                        size="sm" 
                                        icon="share-nodes"
                                        class="backdrop-blur-sm bg-zinc-900/80"
                                    >
                                        <span x-text="copied ? '{{ __('clips.copied') }}' : ''"></span>
                                    </x-ui.button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/30 rounded-2xl p-6">
                            ...
                            <a href="{{ route('clips.list', ['broadcaster' => $clip->broadcaster->twitch_login]) }}" 
                               class="group flex items-center gap-4 p-4 bg-zinc-800/30 rounded-xl hover:bg-zinc-800/50 transition-all">
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-lg font-bold text-zinc-100 group-hover:text-(--color-accent-400) transition-colors truncate">
                                        {{ $clip->broadcaster->twitch_display_name }}
                                    </h4>
                                    <p class="text-sm text-zinc-400 truncate">@{{ $clip->broadcaster->twitch_login }}</p>
                                </div>
                        ...
                        <div class="bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/30 rounded-2xl p-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between p-3 bg-zinc-800/20 rounded-lg">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 bg-(--color-accent-500)/10 rounded-lg flex items-center justify-center">
                                            <i class="fa-solid fa-scissors text-(--color-accent-400) text-sm"></i>
                                        </div>
                                        <span class="text-sm text-zinc-400">{{ __('clips.created_by_label') }}</span>
                                    </div>
                                    <span class="text-zinc-300 font-medium">{{ $clip->clip_creator_name }}</span>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-zinc-800/20 rounded-lg">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 bg-(--color-accent-500)/10 rounded-lg flex items-center justify-center">
                                            <i class="fa-solid fa-user-plus text-(--color-accent-400) text-sm"></i>
                                        </div>
                                        <span class="text-sm text-zinc-400">{{ __('clips.submitted_by_label') }}</span>
                                    </div>
                                    <span class="text-zinc-300 font-medium">{{ $clip->submitter->twitch_login }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-center justify-between p-3 bg-zinc-800/20 rounded-lg">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 bg-(--color-accent-500)/10 rounded-lg flex items-center justify-center">
                                            <i class="fa-solid fa-calendar-plus text-(--color-accent-400) text-sm"></i>
                                        </div>
                                        <span class="text-sm text-zinc-400">{{ __('clips.added_on') }}</span>
                                    </div>
                                    <span class="text-zinc-300 font-medium">{{ $clip->created_at->format('M j, Y') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/30 rounded-2xl p-6">
                            <div class="flex items-center gap-3 mb-6">
                                <div class="w-10 h-10 bg-(--color-accent-500)/10 border border-(--color-accent-500)/20 rounded-lg flex items-center justify-center">
                                    <i class="fa-solid fa-star text-(--color-accent-400)"></i>
                                </div>
                                <h3 class="text-lg font-bold text-zinc-100">{{ __('clips.rating') }}</h3>
                            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
            <div class="flex items-center justify-between mb-8">
                ...
                <a href="{{ route('clips.list', ['broadcaster' => $clip->broadcaster->twitch_login]) }}" 
                   class="text-(--color-accent-400) hover:text-(--color-accent-300) transition-colors">
