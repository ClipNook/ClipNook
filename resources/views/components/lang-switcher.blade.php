...
<div x-data="langSwitcher()" class="relative">
    {{-- Toggle Button --}}
    <x-button variant="icon" type="button" @click="toggle()" @keydown.enter.prevent="toggle()" @keydown.space.prevent="toggle()" x-bind:aria-label="labels.aria_label.replace(':lang', currentLabel)" x-bind:aria-pressed="current !== '{{ app()->getLocale() }}'" aria-haspopup="true" x-bind:aria-expanded="open">
    ...
    <div x-show="open" x-cloak @click.outside="close" @keydown.escape="close"
        ...
        <template x-for="(label, key) in locales" :key="key">
            <button @click.prevent="select(key)" @keydown.enter.prevent="select(key)"
                ...
                x-bind:aria-checked="current === key" x-ref="menuitem"
