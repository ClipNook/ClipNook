            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center py-4">
                    ...
                    <div class="flex items-center">
                        <a href="{{ route('home') }}" class="flex items-center space-x-2 text-white hover:text-gray-300 transition-colors duration-200" aria-label="{{ __('nav.home') }}">
                    ...
                    <!-- Desktop Navigation -->
                    <nav class="hidden md:flex items-center space-x-6" role="navigation" aria-label="{{ __('nav.main_navigation') }}">
                        <a href="{{ route('home') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 {{ request()->routeIs('home') ? 'text-white border-b-2 border-purple-400' : '' }}" aria-current="{{ request()->routeIs('home') ? 'page' : null }}">
                            <i class="fas fa-home mr-1" aria-hidden="true"></i>
                            {{ __('nav.home') }}
                        </a>
                        <a href="{{ route('clips.list') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 {{ request()->routeIs('clips.*') ? 'text-white border-b-2 border-purple-400' : '' }}" aria-current="{{ request()->routeIs('clips.*') ? 'page' : null }}">
                            <i class="fas fa-list mr-1" aria-hidden="true"></i>
                            {{ __('nav.clips') }}
                        </a>
                        @auth
                            <a href="{{ route('clips.submit') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 {{ request()->routeIs('clips.submit') ? 'text-white border-b-2 border-purple-400' : '' }}" aria-current="{{ request()->routeIs('clips.submit') ? 'page' : null }}">
                    ...
                    <div class="flex items-center space-x-4">
                        @auth
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" class="flex items-center space-x-2 text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-gray-900" aria-expanded="false" aria-haspopup="true">
                                    <i class="fas fa-user" aria-hidden="true"></i>
                                    <span class="hidden sm:block">{{ auth()->user()->name }}</span>
                                    <i class="fas fa-chevron-down text-xs" aria-hidden="true"></i>
                                </button>

                                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 mt-2 w-48 bg-gray-800 rounded-md border border-gray-700 z-50" role="menu" aria-orientation="vertical">
                        ...
                        <button
                            x-data="{ open: false }"
                            @click="open = !open"
                            class="md:hidden text-gray-300 hover:text-white p-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-gray-900"
                            aria-expanded="false"
                ...
                <div
                    ...
                    x-cloak
                >
                    <nav class="flex flex-col space-y-2" role="navigation" aria-label="{{ __('nav.main_navigation') }}">
                        <a href="{{ route('home') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 flex items-center {{ request()->routeIs('home') ? 'text-white bg-gray-800' : '' }}" aria-current="{{ request()->routeIs('home') ? 'page' : null }}">
                            <i class="fas fa-home mr-2" aria-hidden="true"></i>
                            {{ __('nav.home') }}
                        </a>
                        <a href="{{ route('clips.list') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 flex items-center {{ request()->routeIs('clips.*') ? 'text-white bg-gray-800' : '' }}" aria-current="{{ request()->routeIs('clips.*') ? 'page' : null }}">
                            <i class="fas fa-list mr-2" aria-hidden="true"></i>
                            {{ __('nav.clips') }}
                        </a>
                        @auth
                            <a href="{{ route('clips.submit') }}" class="text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200 flex items-center {{ request()->routeIs('clips.submit') ? 'text-white bg-gray-800' : '' }}" aria-current="{{ request()->routeIs('clips.submit') ? 'page' : null }}">
