<div class="space-y-6">
    <form wire:submit.prevent="check" novalidate class="space-y-5">
        ...
        <div>
            ...
            <input
                ...
                aria-describedby="clip-help clip-error"
                placeholder="{{ __('clip.submit.labels.placeholder') }}"
        ...
        <div class="flex items-center gap-3">
            <button
                ...
                wire:target="check"
                class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg font-semibold transition-all duration-200
                       {{ $accepted ? 'bg-green-600 hover:bg-green-700' : 'bg-indigo-600 hover:bg-indigo-700' }}
                       text-white 
                       focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500
                       disabled:opacity-70 disabled:cursor-not-allowed">
                
                <svg wire:loading wire:target="check" 
    ...
    @if($message)
        <div role="status" aria-live="polite" class="mt-3 p-3 rounded border text-sm font-medium {{ $accepted ? 'bg-green-50 border-green-200 text-green-800 dark:bg-green-950 dark:border-green-800 dark:text-green-200' : 'bg-red-50 border-red-200 text-red-800 dark:bg-red-950 dark:border-red-800 dark:text-red-200' }}">
    ...
    @if(session()->has('message'))
        <div role="status" 
            ...
            <div class="flex items-center gap-2">
                <i class="fas {{ session('status') === 'success' ? 'fa-check-circle' : 'fa-exclamation-triangle' }}"></i>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="space-y-2 mb-4">
                                <div class="flex flex-wrap items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    @if(!empty($clip['view_count']))
                                        <span class="inline-flex items-center gap-1">
                                            ...
                                            {{ number_format($clip['view_count']) }} {{ __('clip.submit.labels.views') }}
                                        </span>
                                    @endif
                                ...
                                @if(!empty($clip['game_name']))
                                    <div class="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 dark:bg-gray-800 rounded-md">
                            ...
                            <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
                                <a href="{{ $clip['url'] ?? '#' }}" 
                                   ...
                                   class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-colors
                                ...
                                <button 
                                    ...
                                    class="ml-auto inline-flex items-center gap-2 px-5 py-2 text-sm font-bold rounded-lg transition-opacity
                                           bg-green-600 hover:bg-green-700
                                           text-white
                                           disabled:opacity-70 disabled:cursor-not-allowed">
                                    
                                    <svg wire:loading wire:target="saveClip" 
                                         class="w-4 h-4 animate-spin" 
                                         xmlns="http://www.w3.org/2000/svg" 
                                         fill="none" 
                                         viewBox="0 0 24 24"
